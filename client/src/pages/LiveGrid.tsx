...
import { useHeadlessChat, type UseHeadlessChatReturn } from '../hooks/useHeadlessChat';
...
const MiniChat = memo(function MiniChat({
  ...
  const { connected, sending, liveIntent } = chat;

  useEffect(() => { if (isAlive) setForceConnect(false); }, [isAlive]);

  // Wrap chat.send with wake-on-send logic for stopped agents
  const wrappedChat = useMemo<UseHeadlessChatReturn>(() => ({
    ...chat,
    send: (text: string, action?: 'enqueue' | 'steer') => {
      if (shouldConnect) {
        chat.send(text, action);
        return;
      }
      // Agent is stopped — wake it via relay
      setWaking(true);
      chat.setMessages(prev => [
        ...prev,
        { id: `u-wake-${Date.now()}`, role: 'user' as const, text, from: 'you', timestamp: Date.now() },
        { id: `sys-wake-${Date.now()}`, role: 'system' as const, text: '⏳ Waking agent…', timestamp: Date.now() },
      ]);
      api.relayMessage('dashboard', agent.name, text)
  ...
  }), [chat, shouldConnect, agent.name]);

  // Derive status from last message for header indicators
